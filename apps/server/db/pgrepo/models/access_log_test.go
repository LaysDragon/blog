...
func testAccessLogsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(accessLogPrimaryKeyColumns, accessLogColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testAccessLogToOneAccountUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
...
func testAccessLogToOneSetOpAccountUsingUser(t *testing.T) {
	var err error
...
func testAccessLogToOneRemoveOpAccountUsingUser(t *testing.T) {
	var err error
