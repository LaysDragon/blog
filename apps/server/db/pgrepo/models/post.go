...
func (o *Post) GetRelatedAttachtments() AttachtmentSlice {
	...
	return o.R.GetRelatedAttachtments()
}

func (r *postR) GetRelatedAttachtments() AttachtmentSlice {
	if r == nil {
		return nil
	}

	return r.RelatedAttachtments
}
...
var (
	postAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "site_id", "content"}
...
func (o *Post) RelatedAttachtments(mods ...qm.QueryMod) attachtmentQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"attachtment\".\"related_id\"=?", o.ID),
	)
...
// LoadRelatedAttachtments allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (postL) LoadRelatedAttachtments(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	var slice []*Post
	var object *Post
	...
	query := NewQuery(
		qm.From(`attachtment`),
		qm.WhereIn(`attachtment.related_id in ?`, argsSlice...),
	...
	if singular {
		object.R.RelatedAttachtments = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.RelatedID {
				local.R.RelatedAttachtments = append(local.R.RelatedAttachtments, foreign)
				if foreign.R == nil {
					foreign.R = &attachtmentR{}
				}
...
func (postL) LoadComments(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`comment`),
		qm.WhereIn(`comment.post_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddRelatedAttachtments adds the given related objects to the existing relationships
// of the post, optionally inserting them as new records.
// Appends related to o.R.RelatedAttachtments.
...
func (o *Post) AddRelatedAttachtments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Attachtment) error {
	...
	if o.R == nil {
		o.R = &postR{
			RelatedAttachtments: related,
		}
	} else {
		o.R.RelatedAttachtments = append(o.R.RelatedAttachtments, related...)
	}
...
func FindPost(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Post, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"post\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Post) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"post\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func PostExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"post\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
