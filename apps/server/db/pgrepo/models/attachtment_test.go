...
func testAttachtmentToOneAccountUsingOwner(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOwner(ctx, tx, false, (*[]*Attachtment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAttachtmentToOneSetOpAccountUsingOwner(t *testing.T) {
	var err error
...
var (
	attachtmentDBTypes = map[string]string{`ID`: `integer`, `CreatedDate`: `timestamp with time zone`, `UpdatedDate`: `timestamp with time zone`, `OwnerID`: `integer`, `RelatedID`: `integer`, `URL`: `text`}
	_                  = bytes.MinRead
)
