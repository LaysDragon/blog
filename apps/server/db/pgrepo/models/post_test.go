...
func testPostToOneAccountUsingOwner(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOwner(ctx, tx, false, (*[]*Post)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPostToOneSetOpAccountUsingOwner(t *testing.T) {
	var err error
...
var (
	postDBTypes = map[string]string{`ID`: `integer`, `CreatedDate`: `timestamp with time zone`, `UpdatedDate`: `timestamp with time zone`, `OwnerID`: `integer`, `Content`: `text`}
	_           = bytes.MinRead
)
